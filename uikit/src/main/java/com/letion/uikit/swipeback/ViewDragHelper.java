...
public class ViewDragHelper {
     ...
     *                       similar methods invoked as part of layout_autojdkuaibao or drawing.
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    if (mTrackingEdges == EDGE_LEFT && Math.abs(dy) > Math.abs(dx)) {
                        mDragState = STATE_IDLE;
                        break;
                    }

                    if (mTrackingEdges == EDGE_LEFT && dx < 0) {
                        mDragState = STATE_IDLE;
                        break;
                    }

                    if (mTrackingEdges == EDGE_LEFT && mInitialMotionX[pointerId] > TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 100, mContext.getResources().getDisplayMetrics())) {
                        mDragState = STATE_IDLE;
