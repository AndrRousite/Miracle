...
public class SwipeBackLayout extends FrameLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        try {
            if (childId > 0) {
                View pager = findViewById(childId);
                if (pager != null) {
                    int[] location = new int[2];
                    pager.getLocationInWindow(location);
                    int y1 = (int) ev.getY();// 相对于程序最上面
                    int viewHeight = pager.getHeight();
                    // Log.e("pager y", location[1] + viewHeight + "");
                    if (y1 < location[1] + viewHeight) {
                        return false;
                    }
                }
            }
    ...
    public void setChildId(int grid) {
        childId = grid;
    }
